...
<body>
	...
	<div class="container">	
		...
		<?php
		  $id = $_GET['id'];
		  $sql = "SELECT c.comment FROM AnderCat_comments as c  WHERE c.username = '". $username ."' AND c.id = " . $id;
		  $result = $conn->query($sql);
		  if ($result->num_rows > 0) {
		  	while($row = $result->fetch_assoc()) {
		  		?>
		  		<form method="POST" action="./handle_edit.php" class="post">
					<textarea type="textarea" name="message" class="user_message" placeholder="留言"><?php echo $row['comment'] ;?></textarea>
